---
title: "Audit Log: System Activity Tracking in Corpen"
...
| `usuario_id` | `string` | ID of the authenticated user (`Auth::user()->id`) |
| `accion` | `string` | Free-text description of what was done (e.g., `"add usuario app maria.lopez"`) |
| `area` | `string` | Module or area of the application where the action originated (e.g., `"ADMINISTRACIÓN"`, `"CREDITOS"`) |
...
<Tip>
  For compliance reporting, filter the `Auditoria` table by `area` to isolate a specific module's activity, then export the result set to Excel using a library such as `maatwebsite/excel`. Pair the `fechaRegistro` and `horaRegistro` columns to reconstruct a precise chronological timeline. Because `usuario_id` is stored alongside the display name, you can join back to the `users` table even if a user's name has been changed since the event was recorded.
</Tip>
