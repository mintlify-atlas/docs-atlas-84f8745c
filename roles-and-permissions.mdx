---
title: "Role-Based Access Control in Corpen"
sidebarTitle: "Roles & Permissions"
description: "Learn how Corpen implements role-based access using Spatie Laravel Permission, the CanDirect middleware, and admin UI for managing roles and user assignments."
---
...
class User extends Authenticatable
{
    ...
    // Direct permission check (used by CanDirect middleware)
    ...
    // Checks permissions assigned directly to the user (not via role)
    public function hasDirectPermission($permiso): bool
    {
...
`hasDirectPermission()` is the method called internally by the `CanDirect` middleware. It uses Spatie's `getDirectPermissions()` which queries the `model_has_permissions` table — not `role_has_permissions`.
