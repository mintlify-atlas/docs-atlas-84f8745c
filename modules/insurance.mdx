...
| `GET` | `/seguros/reclamacion/informe-completo` | `seguros.reclamacion.exportarInformeCompleto` | Full grouped claims report (Excel, by state) |
...
| `POST` | `/seguros/filtopolizas` | `seguros.poliza.filtros` | Export filtered policies to PDF |
| `POST` | `/seguros/filtopolizas/excel` | `seguros.poliza.filtroexcel` | Export filtered policies to Excel |
...
<Steps>
  ...
  <Step title="Create the third-party and insured records">
    ...
    All three creation steps are recorded in the Auditoría log as separate entries: `TERCERO CREADO`, `ASEGURADO CREADO`, and `POLIZA CREADA`.
  </Step>
</Steps>
...
<CardGroup cols={2}>
  ...
  <Card title="Full Claims Report" icon="file-lines">
    `GET /seguros/reclamacion/informe-completo` — exports claims grouped by state name using `ExportSegInformeReclamacion`, saved as `informe_siniestros.xlsx`.
  </Card>
