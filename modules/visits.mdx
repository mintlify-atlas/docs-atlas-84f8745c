...
sidebarTitle: "Visits"
description: "Log, schedule, and review cooperative member visits with client auto-complete lookup and a centralized visit dashboard in Corpen."
---
...
<Steps>
  ...
  <Step title="Submit the visit record">
    ...
    ```php
    // ScpSoporteController@store validation
...
`VisitaCorpenController@estadisticas` returns a JSON summary of visits grouped by city and by the staff member who recorded them:

```http
GET /visitas/corpen/estadisticas
```

```json
{
  "por_ciudad": [
    { "banco": "Bogotá", "total": 45 },
    { "banco": "Medellín", "total": 20 }
  ],
  "por_usuario": [
    { "registrado_por": "Juan Pérez", "total": 30 }
  ]
}
```

## Searching Visits by Client

`VisitaCorpenController@search` returns all historical visits for a specific member by their `cedula`:

```http
GET /visitas/corpen/search?cedula=10234567
```

This returns an ordered JSON array of `VisitaCorpen` records with the related client loaded.
