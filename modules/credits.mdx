...
<Steps>
  <Step title="Ensure a credit line exists">
    Navigate to `GET /lineas_credito` and verify that the desired product line exists with the correct rate, term, and guarantee type. If not, create one via `GET /lineas_credito/create`. A credit line requires at least one `Garantia` record and one `TipoCredito` record to be selectable in the form.
  </Step>
