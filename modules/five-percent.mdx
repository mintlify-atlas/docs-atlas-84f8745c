...
The `show` action accepts the member's `id` (cédula) as a query parameter rather than a path segment:

```http
GET /cinco/cinco?id={cedula}
```
...
<Steps>
  ...
  <Step title="Search for the member">
    Use the name search at `GET /retiros/retirosname/{name}` or navigate directly to `GET /retiros/calculoretiros/show?id={cedula}`. The system looks up the member in `MaeTerceros` using their `Cod_Ter` and loads their `fec_minis` (ministry entry date) to anchor the liquidation range.
  </Step>
