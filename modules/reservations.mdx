...
|---|---|---|
| `reservas/dashboard` | `ResDashboardController::index` | Reservations dashboard view |
| `reservas/inmueble` | `ResInmuebleController` | Property registry with S3 photo gallery (named `reserva.crudinmuebles.*`) |
...
<Steps>
  ...
  <Step title="Check-in confirmation and rating">
    ...
    Associates can submit their rating via `POST /reservas/reservaI/calificacion` with fields `calificacion` (integer score stored as `puntuacion_asociado`) and `comentario` (stored as `retroalimentacion`). The `fecha_retroalimentacion` is set automatically to today.
  </Step>
</Steps>
